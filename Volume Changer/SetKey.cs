...
namespace Volume_Changer
{
    public partial class SetKey : Form
    {
            ...
            if (UserResponce != DialogResult.Cancel)// The user did not press cancel.
            {
                if (ThisForm.txtButton.Text == Keys.None.ToString())    //The user did not enter a shortcut.
                {
                    DialogResult MessageResult = MessageBox.Show(
                        ...
                        "\nPress OK to specify a shortcut.",
                        "Global Shortcut Example.", MessageBoxButtons.OKCancel,
                        MessageBoxIcon.Question, MessageBoxDefaultButton.Button2);  //The dialog to display to the user.

                    if (MessageResult == DialogResult.OK)// The user wants to go back and specify a shortcut.
                    {
                ...
                {
                    SaveHotkey(ThisForm.txtButton.Text);
        ...
        public static void SaveHotkey(string text)
        {
            Properties.Settings.Default.KeyboardShortcut = text;
